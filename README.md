...
> Install once, keep five AI agent hosts in sync.

### What It Solves

Claude, Codex, Gemini, Kiro, and `.agents` all keep skills in different directories. Manual copying creates missed installs, drift between hosts, and maintenance overhead.
...
| **5-host support** | Claude, Codex, Gemini, Kiro, and `.agents` |
...
| **Cross-platform** | Uses native symlinks where safe, `cp -r` for Claude and Kiro (Kiro doesn't support symlinks), Python `os.symlink` on Windows, and `force_remove_dir` for Python 3.12+ rmtree compatibility |
...
Entry B: Sync from Claude to other hosts
  ...
  Flow: detect new skill -> sync to other 4 hosts (Kiro via cp -r, others via symlink) -> update catalog -> regenerate INSTALLED.md -> prompt provenance check
...
~/.kiro/skills/                -> cp -r copies (no symlink support)
~/.agents/skills/              -> same pattern as Codex
```
...
- The other 4 hosts receive standalone copies instead of symlinks.
- This avoids coupling non-Claude hosts to marketplace-specific behavior.
- Kiro always receives `cp -r` copies regardless of marketplace detection.
...
```text
install-skills/
├── SKILL.md                    # Main skill (routing + Entry A/B/C/D)
├── README.md
├── assets/
│   ├── installed-md.png
...
├── references/
│   ├── advanced-entries.md     # Entry E/F/G/H detailed steps (lazy-loaded)
│   ├── shared-platform.md      # Cross-platform compat + pitfalls (lazy-loaded)
│   ├── catalog-skeleton.md     # SKILLS-CATALOG strict schema
│   └── origin-schema.md
└── scripts/
    └── gen_installed.py
```
...
| Use `cp -r` for Kiro | Create symlinks for Kiro | Kiro cannot recognize symlinked skills |
...
| Verify CATALOG has all 8 skeleton elements after every update | Only update category tables | Missing provenance table or overview table breaks Entry E/F |
...
> 装一次，让五个 AI agent host 保持同步。

### 它解决什么问题

Claude、Codex、Gemini、Kiro 和 `.agents` 都有各自的 skill 目录。手动复制到 5 个地方，常见结果就是漏装、版本漂移、后续维护失控。
...
| **支持 5 个 host** | Claude、Codex、Gemini、Kiro、`.agents` |
...
| **跨平台** | Claude 和 Kiro 用 `cp -r` 实体复制（Kiro 不支持 symlink），其余 host macOS/Linux 用符号链接、Windows 用 Python `os.symlink`，`force_remove_dir` 兼容 Python 3.12+ |
...
入口 B：从 Claude 同步到其它 host
  ...
  流程：识别新 skill -> 同步到其它 4 个 host（Kiro 用 cp -r，其余用符号链接）-> 更新目录 -> 重建 INSTALLED.md -> 提示溯源排查
...
~/.kiro/skills/                -> cp -r 实体复制（不支持 symlink）
~/.agents/skills/              -> 同 Codex 模式
```
...
- 另外 4 个 host 使用独立副本，而不是符号链接。
- 这样可以避免把官方安装行为和本地链接策略强耦合。
- Kiro 无论是否走 marketplace，始终使用 `cp -r` 实体复制。
...
```text
install-skills/
├── SKILL.md                    # 主 skill（路由 + 入口 A/B/C/D）
├── README.md
├── assets/
│   ├── installed-md.png
│   ├── skills-catalog-detail.png
│   └── skills-catalog-overview.png
├── references/
│   ├── advanced-entries.md     # 入口 E/F/G/H 详细步骤（按需加载）
│   ├── shared-platform.md      # 跨平台兼容 + 常见踩坑（按需加载）
│   ├── catalog-skeleton.md     # SKILLS-CATALOG 骨架模板（强制 schema）
│   └── origin-schema.md
└── scripts/
    └── gen_installed.py
```
...
| 每次更新 CATALOG 后检查骨架 8 项齐全 | 只改分类表不管其他结构 | 缺少溯源表/总览表/快速指南会导致入口 E/F 失效 |
